...
class OptimizedMainController(object):
    def __init__(self):
        ...
        # 区分首次导航和返回导航
        self.task_phase = "initial"  # initial, returning, completed
        ...
        rospy.loginfo("OptimizedMainController initialized with SMART PHASE TRACKING")
    ...
    def debug_status(self, event):
        ...
        rospy.loginfo("Phase: %s | Packages: Expected=%s | Actual=%s | Last=%s | Done=%d/%d", 
                     self.task_phase, self.current_package_color, self.actual_package_color,
    ...
    def send_navigation_command(self, command):
        """智能导航命令发送 - 根据任务阶段选择正确命令"""
        if command == "goto_pickup_approach":
            if self.task_phase == "returning" and self.last_placed_color:
                # 如果是返回阶段且有上次放置的颜色，使用返回命令
                actual_command = "return_from_{}_to_pickup".format(self.last_placed_color)
                rospy.logwarn("🔧 PHASE CORRECTION: {} -> {} (phase: {})", 
                             command, actual_command, self.task_phase)
                command = actual_command
            elif self.task_phase == "initial":
                # 初次导航使用正常接近命令
                rospy.logwarn("🔧 INITIAL PHASE: Using pickup approach")
        
        cmd_msg = String()
        cmd_msg.data = command
        self.navigation_cmd_pub.publish(cmd_msg)
        rospy.logwarn("🚀 Navigation command: %s from [%.2f, %.2f] (phase: %s)", 
                     command, self.current_position[0], self.current_position[1], self.task_phase)
    ...
    def is_pickup_approach_completed(self):
        ...
        completed_statuses = [
            ...
            "arrived_return_to_pickup", "timeout_return_to_pickup"  # 包含返回状态
        ]
        position_check = (
    ...
    def handle_navigate_to_pickup_state(self):
        ...
        if not self.is_navigation_active():
            rospy.loginfo("Starting navigation to pickup from [%.2f, %.2f] (phase: %s)", 
                         self.current_position[0], self.current_position[1], self.task_phase)
            
            # 根据任务阶段发送不同的导航命令
            if self.task_phase == "returning" and self.last_placed_color:
                # 返回阶段：使用返回命令
                command = "return_from_{}_to_pickup".format(self.last_placed_color)
                rospy.logwarn("🔧 RETURNING PHASE: Using return command: %s", command)
                self.send_navigation_command(command)
            else:
                # 初始阶段：使用接近命令
                rospy.logwarn("🔧 INITIAL PHASE: Using approach command")
    ...
    def handle_place_package_state(self):
        ...
        if self.manipulation_status == "place_completed":
            ...
            self.last_placed_color = self.last_placed_package_color 
            self.placement_completed_time = time.time()
            self.task_phase = "returning"  # 设置为返回阶段
            self.packages_handled += 1
            
            rospy.logwarn("🔧 TASK PHASE: initial -> returning (placed: %s)", self.last_placed_color)
            ...
            if self.completed_packages >= self.target_packages:
                ...
                rospy.logwarn("FAST TRANSITION: Package %d/%d done, returning with correct command", 
                             self.completed_packages, self.target_packages)
                # 进入返回状态而不是NAVIGATE_TO_PICKUP
    ...
    def run(self):
        ...
        rospy.loginfo("Starting OPTIMIZED warehouse robot with SMART PHASE TRACKING")
