...
class LocationChecker(object):
    def __init__(self):
        ...
        # 🔥 根据warehouse.world文件的正确配置 - 放宽区域判断
        self.pickup_zone = rospy.get_param('/pickup_zone', {
            'center': [0.0, -1.5],
            'radius': 1.5,  # 增加半径
            'safe_exit_point': [0.0, 1.5]
        })
        
        # 🔥 放宽的drop区域配置 - 增加半径避免误判
        self.drop_zones = rospy.get_param('/drop_zones', {
            'red': {'center': [1.0, 5.0], 'radius': 1.8},    # 从1.2增加到1.8
            'blue': {'center': [3.0, 5.0], 'radius': 1.8},   # 从1.2增加到1.8
            'green': {'center': [-5.0, 1.0], 'radius': 1.8}, # 从1.2增加到1.8
            'purple': {'center': [5.0, 1.0], 'radius': 1.8}  # 从1.2增加到1.8
        })
        ...
        if 'safe_exit_point' not in self.pickup_zone:
            self.pickup_zone['safe_exit_point'] = [0.0, 1.5]
            rospy.logwarn("Adding default safe_exit_point: [0.0, 1.5]")
        ...
        # 🔥 移除AMCL误差校正 - 使用Gazebo地面真实坐标
        self.amcl_correction_x = 0.0
        self.amcl_correction_y = 0.0  # 移除手动校正
    ...
    def detect_current_zone(self):
        ...
        # 🔥 检查pickup区域 - 使用更宽松的矩形检测
        # pickup_area在world文件中是3x2米，中心在[0, -1.5]
        pickup_bounds = {
            'x_min': -2.0,  # 从-1.8放宽到-2.0
            'x_max': 2.0,   # 从1.8放宽到2.0
            'y_min': -3.0,  # 从-2.8放宽到-3.0
            'y_max': 0.5    # 从-0.2放宽到0.0
        }
